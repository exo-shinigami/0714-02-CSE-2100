...
static const char* GetPieceSymbol(int piece) {
    switch(piece) {
...
// Cross-platform helper to open a font with sensible fallbacks.
static TTF_Font* openFont(const char* preferredPath, int ptsize) {
    TTF_Font* font = NULL;
    if (preferredPath) {
        font = TTF_OpenFont(preferredPath, ptsize);
        if (font) return font;
    }
    ...
    // Try fontconfig (fc-match) as a last resort to locate a usable font file
#if !defined(_WIN32) && !defined(_WIN64)
...
void renderGameOverMessage(GUI* gui) {
    ...
    TTF_Font* font = openFont(NULL, 36);
    if (!font) {
...
void renderGameMode(GUI* gui) {
    ...
    TTF_Font* font = openFont(NULL, 18);
    if (!font) {
...
void renderPromotionDialog(GUI* gui, ChessBoard* board) {
    ...
    // Title
    TTF_Font* titleFont = openFont(NULL, 24);
    if (titleFont) {
    ...
    TTF_Font* pieceFont = openFont("C:/Windows/Fonts/seguisym.ttf", 50);
    ...
    // Draw labels
    TTF_Font* labelFont = openFont(NULL, 14);
    if (labelFont) {
...
static void gUIDrawPiece(GUI* gui, int piece, int x, int y) {
    ...
    TTF_Font* font = openFont("C:/Windows/Fonts/seguisym.ttf", 60);
    if (!font) return;
...
static void drawCapturedPiece(GUI* gui, int piece, int x, int y, int size) {
    // Safety check for piece bounds
    if (piece == EMPTY || piece == OFFBOARD || piece < 0 || piece >= 13) {
        return;
    }
    ...
    TTF_Font* font = openFont("C:/Windows/Fonts/seguisym.ttf", size - 5);
    if (!font) return;
...
static void renderCapturedPieces(GUI* gui) {
    ...
    TTF_Font* font = openFont(NULL, 16);
...
static void renderTimers(GUI* gui, ChessBoard* board) {
    TTF_Font* font = openFont(NULL, 24);
    if (!font) return;

    TTF_Font* labelFont = openFont(NULL, 16);
...
static void renderMoveHistory(GUI* gui, ChessBoard* board) {
    ...
    // Load font for moves
    TTF_Font* font = openFont(NULL, 14);
    if (!font) return;
