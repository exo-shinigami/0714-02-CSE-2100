...
The original codebase had all 30+ source files in the root directory with build artifacts, DLLs, scripts, and documentation mixed together. This has been resolved.
...
│   │   └── gambit.c              # Main function (GUI launch + CLI args)
│   │
│   ├── core/                      # Core chess logic (no UI dependencies)
│   │   ├── types/                # Type definitions
│   │   │   ├── defs.h            # Common type definitions and macros
│   │   │   └── data.c            # Global data tables
│   │   │
│   │   ├── board/                # Board representation
│   │   │   ├── board.c           # Board state management
│   │   │   ├── hashkeys.c        # Zobrist hash key generation
│   │   │   └── validate.c        # Board state validation
│   │   │
│   │   ├── moves/                # Move generation and execution
│   │   │   ├── movegen.c         # Move generation
│   │   │   ├── makemove.c        # Move execution
│   │   │   └── io.c              # Move I/O formatting
│   │   │
│   │   ├── bitboards/            # Bitboard operations
│   │   │   └── bitboards.c       # Bitboard utilities
│   │   │
│   │   └── attack/               # Attack detection
│   │       └── attack.c          # Square attack detection
│   │
...
│   │       └── pvtable.c         # Principal variation table
│   │
│   ├── ui/                        # User interface layer
│   │   ├── console/              # Console UI (empty - console mode removed)
│   │   │
│   │   ├── sdl/                  # SDL2 GUI
│   │   │   ├── gui.h             # GUI header
...
│       ├── polykeys.h            # Polyglot key definitions
│       ├── polybook.c            # Polyglot book reader
│       └── polykeys.c            # Polyglot key data
│
├── build/                         # Build output (gitignored)
│   ├── obj/                      # Object files (mirrors src/ structure)
│   └── bin/                      # Executable (copied to root)
│       └── gambit.exe
│
├── lib/                          # SDL2 libraries (reference only)
│   ├── SDL2.dll                  # 64-bit SDL2 library (for dynamic builds)
│   └── SDL2_ttf.dll              # 64-bit SDL2_ttf library (for dynamic builds)
│
├── scripts/                       # Build and utility scripts
│   ├── setup_portable.bat        # Windows portable build script
│   └── setup_portable.sh         # Unix portable build script
│
├── gambit.exe                    # Statically-linked executable (~7MB)
...
### Phase 3: File Structure Reorganization (Week 3) ✅ COMPLETED

**Status:** Done. All source files have been moved to the modular `src/` directory structure. Build artifacts go to `build/`, libraries to `lib/`, scripts to `scripts/`, and documentation to `docs/`. The makefile has been updated to reference the new paths.

**Additional Updates:**
- Console mode removed - game launches directly into GUI by default
- SDL2 statically linked - DLLs in `lib/` are reference only, not required at runtime
- Single executable `gambit.exe` (~7MB) in root directory for easy access
- Command-line arguments supported: `gambit.exe uci` or `gambit.exe xboard` for protocol modes

**Completed Structure:**
```
src/core/types/       (foundation)
src/core/board/       (board representation)
src/core/bitboards/   (bitboard operations)
src/core/attack/      (attack detection)
src/core/moves/       (move generation/execution)
src/engine/           (search, evaluation, hashtable)
src/ui/sdl/           (SDL2 GUI - statically linked)
src/ui/protocols/     (UCI/XBoard protocols)
src/utils/            (initialization, misc utilities)
src/openingbook/      (polyglot book support)
lib/                  (SDL2 DLLs - for reference/dynamic builds only)
scripts/              (portable build scripts)
